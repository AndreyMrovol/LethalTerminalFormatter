  ...
  public class MoonsNoLLL : TerminalFormatterNode
  {
    ...
    public override string GetNodeText(TerminalNode node, Terminal terminal)
    {
      ...
      List<MrovLib.ContentType.Route> routes = ContentManager
        ...
        .OrderBy(keyval => Settings.MoonsOrderVanilla.IndexOf(MrovLib.SharedMethods.GetNumberlessPlanetName(keyval.Level)))
        .ToList();
      ...
      foreach (MrovLib.ContentType.Route route in routes)
      {
        ...
        string moonName = ConfigManager.ShowNumberedPlanetNames.Value ? level.PlanetName : MrovLib.SharedMethods.GetNumberlessPlanetName(level);
        ...
        table.AddRow(moonName, $"${price}", SharedMethods.GetWeather(level).PadRight(Settings.planetWeatherWidth));
      ...
      adjustedTable.Append($" The Company is buying at {Mathf.RoundToInt(StartOfRound.Instance.companyBuyingRate * 100f)}%. \n\n");
