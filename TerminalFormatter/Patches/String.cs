...
namespace TerminalFormatter.Patches
{
  ...
  {
    public static string Sanitized(this string currentString) => new(currentString.SkipToLetters().RemoveWhitespace().ToLowerInvariant());

    public static string RemoveWhitespace(this string input) => new(input.ToCharArray().Where(c => !Char.IsWhiteSpace(c)).ToArray());

    public static string SkipToLetters(this string input) => new(input.SkipWhile(c => !char.IsLetter(c)).ToArray());
